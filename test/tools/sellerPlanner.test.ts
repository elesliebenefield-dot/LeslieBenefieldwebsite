...
async function fillStep4(page: Page) {
  ...
  await pickRadio(page, 'multipleOwners', 'no')
...
test('page renders the header and step 1 questions', async () => {
  ...
  try {
    ...
    assert.match(brand!, /Websites by Leslie/i)
...
test('results include disclaimer with real estate advice and EHO text', async () => {
  const page = await openPage()
  try {
    await completeAllSteps(page)
    ...
    assert.match(disclaimer, /Equal Housing Opportunity/i)
...
test('Start Over button shows a confirmation dialog', async () => {
  ...
  try {
    ...
    await page.click('.tool-start-over-btn')
