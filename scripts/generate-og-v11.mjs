...
/* Content block — centered slightly above vertical midpoint */
.content {
  position: absolute;
  left: 0;
  right: 0;
...
try {
  ...
  const out = join(__dirname, '..', 'public', 'social-preview-v11.jpg');
  writeFileSync(out, jpg);
  console.log(`Saved: public/social-preview-v11.jpg  (${(jpg.length / 1024).toFixed(1)} KB)`);
} finally {
  await browser.close();
  unlinkSync(tmpHtml);
}
